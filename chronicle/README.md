...
- **Live edition:** published to a private Claude artifact. The URL is held outside this repo.
...
| Personal cal | Google Calendar MCP | **Personal only** — tennis, family, personal appointments, social events. Never the source of truth for the work week; merge into the Almanac as personal items, clearly. Do not report a work meeting "missing from the calendar" based on Google alone — check Outlook first |
...
8. **The Homestead** — standing section (added 2026-07-17 at the publisher's
   ...
   the work is complete. Read the contractor and insurer threads in Gmail —
   inbox AND Sent — each edition. Report: what's settled, what's open, and a
   **days-since-last-reply counter for every unanswered outbound thread**
   (silence is the story). Names, claim
...
- **Reconcile the Docket against Sent mail before ranking it.** The `tasks`
  ...
  **Sent Items in both mailboxes** (Outlook `CREED@MCTVOFMS.COM` via the M365
  ...
  produced a wrong edition on 2026-07-16: two client decks, both sent the
  week prior, were reported as still owed after they had shipped, because the
  desks read the inbox but never the Sent folder.
...
- **Mississippi Live Weather partnership** — a cross-network distribution
  deal (storm-season audience ⇄ MCTV's 125+ screens). Logged in
  `pipeline_opportunities`. Pitch deck and materials are in active
  production. Report deal state from the pipeline row; never
